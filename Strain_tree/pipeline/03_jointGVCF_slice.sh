...
if [ ! -f $GENOVCFOUT.gz ]; then
	...
	rm -rf $DB
	gatk  --java-options "-Djava.io.tmpdir=/scratch -Xmx$MEM -Xms$MEM" GenomicsDBImport --consolidate --merge-input-intervals --genomicsdb-workspace-path $DB $FILES $INTERVALS --tmp-dir $TEMP 
	#--reader-threads $CPU
	#gatk  --java-options "-Xmx$MEM -Xms$MEM" GenomicsDBImport --genomicsdb-workspace-path $DB $FILES $INTERVALS  --reader-threads $CPU
	time gatk --java-options "-Djava.io.tmpdir=/scratch -Xmx$MEM -Xms$MEM" GenotypeGVCFs --reference $REFGENOME --output $GENOVCFOUT -V gendb://$DB --tmp-dir $TEMP
	ls -l $TEMP
	rm -rf $DB
    fi
    if [ -f $GENOVCFOUT ]; then
